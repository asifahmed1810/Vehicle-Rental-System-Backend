...
const deleteVehicle=async(id:string)=>{
    const checkAvailable=await pool.query(`SELECT availability_status FROM vehicles WHERE id=$1`,[id])

    if(checkAvailable.rows.length ===0){
        return {notFound:true}
    }

    const status=checkAvailable.rows[0].availability_status;
    ...
    return {deleted:result.rows[0], result}
